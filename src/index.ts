...
import { calculateBmi, type CalculateBmiResult } from "./lib/calculate-bmi.ts";
import {
  calculateExercises,
  type CalculateExercisesResult,
} from "./lib/calculate-exercises.ts";
...
app.get(
  ...
  (
    req,
    res: Response<
      | { weight: number; height: number; bmi: CalculateBmiResult }
      | { error: string }
    >,
  ) => {
...
app.post(
  "/exercises",
  (req, res: Response<CalculateExercisesResult | { error: string }>) => {
